...
const Verification_id = () => {
  ...
  const Check_Access_Id = async () => {
    ...
    if (count === 0) {
      document.getElementById("below_input_message").innerHTML =
        "Invalid Access Id 🙄";
      ...
      <div className="delete_content">
        <div className="Verification_input-field">
          <input
            ...
            name="Access_id"
            required
            onChange={handleChange_Access_id}
          />
          <i class="uil uil-keyhole-square-full"></i>
        </div>
        <h1 id="below_input_message">Enter your Access ID for verification!</h1>
        <span>
          <Button
            positive
            className="delete_cancle"
            disabled={Access_id == ""}
            onClick={() => Check_Access_Id()}
          >
            Submit
          </Button>
          <Button
