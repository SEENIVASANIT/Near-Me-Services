...
const Professions_post = ({ setOpen3, id, collections, name, email }) => {
  ...
  const [Access_id_post, setaccessid] = useState("");
    ...
    if (!localStorage.getItem(id + "post_report" + pass_val.login_user_email)) {
      try {
        ...
        localStorage.setItem(
          id + "post_report" + pass_val.login_user_email,
          id
        );
  ...
  const handleAutoDelete_post = (id) => {
    if (localStorage.getItem(id + "heart" + pass_val.login_user_email)) {
      ...
      localStorage.getItem(id + "post_report" + pass_val.login_user_email)
    ) {
      localStorage.removeItem(id + "post_report" + pass_val.login_user_email);
    }
      ...
      <div className="comment_popup">
        <div className="comment_box1">
          ...
          <div class="message1">
            ...
            >
              This worker has not uploaded any post yet!
            </h1>
            {users.map((item) => {
              return (
                <div className="all_comments1">
                  <div id="edit-delete-poster_info">
                    ...
                    <span></span>
                    {localStorage.getItem("Access_id") == item.Post_id ? (
                      <i
                        className="fas fa-edit"
                        id="post-edit"
                        onClick={() => update_post(item.id, item.img)}
                      ></i>
                    ) : (
                      <></>
                    )}
                  ...
                  <a href={item.img} target="_blank">
                    <img src={item.img} id="post_image"></img>
                  </a>

                  <div className="like-dislike-commet-report">
                    {localStorage.getItem(
                      item.id + "heart" + pass_val.login_user_email
                    ) === item.id ? (
                      <i
                        className="fas fa-regular fa-heart"
                        id="heart1"
                        onClick={() => handleLikes(item.id, item.like)}
                      ></i>
                    ) : (
                      <i
                        className="fas fa-regular fa-heart"
                    ...
                    <span id="post_like_count"> {"Likes:" + item.like}</span>
                    <i
                      className="fas fa-comment-dots"
                      id="post_comment"
                      onClick={() => post_comments(item.id)}
                    ></i>
                    <splan id="post_comment_count"> {"Comments"}</splan>
                    {localStorage.getItem(
                      item.id + "post_report" + pass_val.login_user_email
                    ) === item.id ? (
                      <i
                    ...
                    <span id="post_report_count">Report.{item.report}</span>
                    {item.report >= 50 ? handleAutoDelete_post(item.id) : <></>}
                    {open6 && (
                      <Comments
                        open1={open6}
                        setOpen1={setOpen6}
          ...
          {pass_val.login_user_email == email ||
          pass_val.login_user_email === "20110090@hicet.ac.in" ? (
            <Button primary id="add_post" onClick={() => setOpen4(true)}>
              Add post
            </Button>
          ) : (
            <></>
