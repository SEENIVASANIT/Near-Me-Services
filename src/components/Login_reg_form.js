...
const Login_reg_form = ({ setOpen }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    await users.map(async (item) => {
      if (item.user_email === user_datas.get("user_email")) {
        check_mail = false;
  ...
  const handleLogin = async (e) => {
    ...
    await users.map((item) => {
      ...
      if (
        ...
        const current_user = {
          login_user_email: user_datas.get("login_user_email"),
          login_user_passw: user_datas.get("login_user_passw"),
        };
        localStorage.setItem("current_user", JSON.stringify(current_user));
        if (
          user_datas.get("login_user_email") === "20110090@hicet.ac.in" &&
          user_datas.get("login_user_passw") === "26@#seeni"
        ) {
