...
const Comments = ({ open1, setOpen1, id, collections, name }) => {
  ...
  const handleDelet = async (id) => {
    ...
    if (id === null) {
      setOpen(false);
      if (
        localStorage.getItem(get_delete_id + "like" + pass_val.login_user_email)
      ) {
        localStorage.removeItem(
          get_delete_id + "like" + pass_val.login_user_email
        );
      } else if (
        localStorage.getItem(
          get_delete_id + "dislike" + pass_val.login_user_email
        )
      ) {
        localStorage.removeItem(
          get_delete_id + "dislike" + pass_val.login_user_email
        ...
        localStorage.getItem(
          get_delete_id + "report" + pass_val.login_user_email
        )
      ) {
        localStorage.removeItem(
          get_delete_id + "report" + pass_val.login_user_email
  ...
  const handleAutoDelete = (id) => {
    if (localStorage.getItem(id + "like" + pass_val.login_user_email)) {
      ...
      localStorage.getItem(id + "dislike" + pass_val.login_user_email)
    ) {
      localStorage.removeItem(id + "dislike" + pass_val.login_user_email);
    } else if (
      localStorage.getItem(id + "report" + pass_val.login_user_email)
    ) {
      localStorage.removeItem(id + "report" + pass_val.login_user_email);
    }
    ...
    <>
      <div className="comment_popup">
        <div className="comment_box">
          ...
          <div class="message">
            <h2 id="no_comment">Be the first one to post your comment!</h2>
            {!loading ? <Spinner /> : <></>}
              ...
              return (
                <div id="all_comments">
                  <div>
                    ...
                    <h3>
                      ...
                      <span id="comment_user_name">
                        {item.name} . {item.commend_post_date}
                        <span id="comment_post_time"> at {item.Time}</span>
                      </span>
                    </h3>
                  </div>
                  <span>
                    {item.name == pass_val.current_user_name ||
                    (pass_val.login_user_email === "20110090@hicet.ac.in" &&
                      pass_val.login_user_passw === "26@#seeni") ? (
                      <i
                        className="fas fa-edit "
                        id="edit_icon_for_comment"
                        onClick={() => handleUpdate(item.id, item.comment)}
                      ></i>
                    ) : (
                      console.log()
                    )}
                  ...
                  <div id="lik-dis">
                    <span>
                      {localStorage.getItem(
                        item.id + "like" + pass_val.login_user_email
                      ) === item.id ? (
                        <i
                          className="fas fa-thin fa-thumbs-up"
                          id="like1"
                          onClick={() =>
                            handleLike(item.id, item.like, item.dislike)
                          }
                        ></i>
                      ) : (
                        <i
                      ...
                      <span id="total_likes"> {item.like}</span>
                      {localStorage.getItem(
                        item.id + "dislike" + pass_val.login_user_email
                      ) === item.id ? (
                        <i
                          className="fas fa-thin fa-thumbs-down"
                          id="dislike1"
                          onClick={() =>
                            handleDisLike(item.id, item.dislike, item.like)
                          }
                        ></i>
                      ) : (
                        <i
                      ...
                      <span id="total_dislikes"> {item.dislike}</span>
                      {localStorage.getItem(
                        item.id + "report" + pass_val.login_user_email
                      ) === item.id ? (
                        <i
                          className=" fas fa-regular fa-flag"
                          id="report1"
                          onClick={() => handelReport(item.id, item.report)}
                        ></i>
                      ) : (
                        <i
                          className=" fas fa-regular fa-flag"
          ...
          {isSubmit ? (
            ...
            <form onSubmit={handleSubmit}>
              <div class="input-comment_container">
                ...
                />
                <Button primary id="send" disabled={comment == ""}>
